...
from src.models.passt.passt_feature_extraction import PasstCNNFeatureExtractor


class PasstComplexCNN(PaSST_SED):

    def __init__(self, passt_sed_param, cnn_param) -> None:
        super().__init__(**passt_sed_param)
        self.mel_trans = PasstCNNFeatureExtractor(
            n_mels=128,
            sr=32000,
            win_length=800,
            hopsize=320,
    ...
    def forward(
        ...
        if encoder_win:
            ...
            x_local = self.slide_window_layer(slide_window_model(input['trans'], emb_len=x.shape[1]))
            x = mix_rate * x_local + (1 - mix_rate) * x

        # merge CNN's feature
        cnn_feat = self.cnn(input['cnn'].transpose(2, 3))
