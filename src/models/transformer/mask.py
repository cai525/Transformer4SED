...
class MlmModule:
    def __init__(self, mask_rate=0.15, mask_style=(0.8, 0.1, 0.1), strategy = "random", 
                 block_width=10, device=None, *arg, **kwarg) -> None:
    ...
    def setence_mask(self, token_seq:Tensor, mask_token:Tensor):
        ...
        # Flatten the token sequence for easier manipulation
        ...
        random_mask = mask_id_flat & (probs >= self.mask_style["mask_token"]) & (probs < self.mask_style["mask_token"] + self.mask_style["random"])
        ...
        # Note: No operation needed for keeping the original token (self)
