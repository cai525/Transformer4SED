...
from src.models.atst.transformer import Block
...
class FrameAST(nn.Module):
    ...
    def prepare_tokens(self, x, mask_index, length, mask=True):
        ...
        # mask the tokens specificed by mask_index
    ...
    def forward(self, x, mask_index=None,mask_input=True,length=None):
        ...
        # make sure that the elements in the mask_index are less than patch_length 
    ...
    def get_intermediate_layers(self, x,length, n=1, scene=True, other_emb=None):
        ...
        for i,blk in enumerate(self.blocks):
            ...
            if len(self.blocks) - i <= n :
                ...
                if scene:
                    # get token for audio scene classification
                    ...
                    output_dict["avg_token_{layer}".format(layer=i+1)] = avg
                    if self.nprompt>0:
                        output_dict["prompt_{layer}".format(layer=i+1)] = torch.mean(x[:,:self.nprompt],dim=1)
                else:
                    output_dict["feature_map_{layer}".format(layer=i+1)] = norm_x[:,self.nprompt:]
