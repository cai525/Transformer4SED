...
class PasstWithSlide(EncoderSlideWindow):
    ...
    def encode(self, input: torch.Tensor) -> torch.Tensor:
        ...
        passt_out_dict = self.net.get_encoder()(input)
        ...
        if self.net.get_decode_ratio() != 1:
            frameout = frameout.transpose(1, 2)  # B,T,C->B,C,T
            frameout = F.interpolate(frameout, scale_factor=self.net.get_decode_ratio(), mode=self.net.interpolate_module.mode)
