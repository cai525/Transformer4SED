...
from recipes.desed.setting import get_logger
from src.preprocess.dataset import StronglyLabeledDataset, UnlabeledDataset
...
def get_save_directories(configs, save_folder):
    ...
    best_path = os.path.join(save_folder, "best_model.pt")
    configs["training"]["best_paths"] = best_path

    # psds folder
    if configs["generals"]["savepsds"]:
        stud_psds_folder = os.path.join(save_folder, "psds")
        psds_folder = stud_psds_folder
    else:
        psds_folder = None

    configs["training"]["psds_folder"] = psds_folder
    configs["training"]["save_folder"] = save_folder

    return configs


def get_encoder(config):
    with open(config['dataset']['label_dict_path']) as f:
...
def get_weighted_sampler(dataset: StronglyLabeledDataset, weight_path, num_samples=100000):
    df = pd.read_csv(weight_path, sep='\t', index_col="filename")
    weight = [df.loc[filename, 'weight'] for filename in dataset.clip_list]
    sampler = WeightedRandomSampler(weight, num_samples=num_samples)
    return sampler
...
def prepare_run():
    ...
    parser.add_argument('--open_vocabulary', type=bool, default=False)
...
def get_datasets(config, encoder: Encoder, evaluation, test_only, logger):
    ...
    if not test_only:
        assert config["training"]["batch_size"] % int(torch.cuda.device_count()) == 0, \
        "The training batch size(={0}) must be set to an integer multiple of the number of GPUs(={1})"\
            .format(config["training"]["batch_size"], int(torch.cuda.device_count()))

        ##### data prep train & valid ##########
        train_strong_df = pd.read_csv(config["dataset"]["strong_tsv"], sep="\t")
        # train_strong_df = train_strong_df[:3600] # for debug
        ...
        # logging
        logger.info("train={0}, val={1}, test={2}".format(len(train_dataset), len(valid_dataset), len(test_dataset)))
    else:
        train_dataset = None
        valid_dataset = None
        train_dataset = None
...
def dataset_setting(encoder, configs, logger):
    # get dataset
    train_dataset, valid_dataset, test_dataset = get_datasets(configs,
                                                              encoder,
    ...
    if not configs["generals"]["test_only"]:
        sampler = get_weighted_sampler(train_dataset, weight_path=configs["dataset"]['reweight_path'])
...
def optimizer_and_scheduler_setting(lr, train_loader, configs, logger):
    ...
    optimizer = torch.optim.AdamW(lr, **optim_kwargs)
...
class BestModels:
    """ 
    Class to keep track of the best  models and save them after training
    """

    def __init__(self):
        self.best_val_metric = -0.00001
        self.best_state_dict = None

    def update(self, net, epoch, logger, val_metrics):
        update = False
        if val_metrics > self.best_val_metric:
            self.best_val_metric = val_metrics
            self.best_state_dict = copy.deepcopy(net.state_dict())
            update = True
        if epoch >= 0:
            if update:
                logger.info("     best model updated at epoch %d!" % epoch)
        return

    def save_bests(self, best_path):
