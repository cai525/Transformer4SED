...
from recipes.audioset_strong.setting import load_yaml_with_relative_ref, get_save_directories, get_logger
...
class OpensetClapEvaluator(HTSAT_CNN_Trainer):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def openset_eval(self, query: torch.Tensor, openset_label_list):
        self.net.eval()
...
if __name__ == "__main__":
    ...
    with open(args.openset_label) as f:
        openset_label_list = json.load(f)
    label_list = label_list + openset_label_list
    ...
    query = torch.cat((net.text_query, torch.load(args.openset_embedding)))
    logging.info("<INFO> shape of query  is {}".format(query.shape))

    # set dataloader
    test_df = pd.read_csv(configs["dataset"]["openset_tsv"], sep="\t")
    ...
    evaluator = OpensetClapEvaluator(
        optimizer=None,
        my_logger=my_logger,
        net=net,
        config=configs,
    ...
    evaluator.openset_eval(query, openset_label_list)
