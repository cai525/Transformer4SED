import argparse
import logging
import os
import sys
import yaml

import torch
import torch.nn as nn
from tqdm import tqdm
from torch.utils.data import DataLoader

root = "/home/cpf/code/SSL_SED"
os.chdir(root)
sys.path.append(root)

from recipes.audioset_strong.setting import *
from recipes.desed.detect_any_sound.detect_any_sound.finetune.setting import get_logger
from recipes.audioset_strong.detect_any_sound.passt.train import DASMTrainer
from src.codec.encoder import Encoder
from src.codec.decoder import batched_decode_preds
from src.models.detect_any_sound.detect_any_sound import DASM
from src.utils.statistics.model_statistic import count_parameters
from src.utils import load_yaml_with_relative_ref


def prepare_run():
    torch.backends.cudnn.benchmark = True
    # parse the argument
    parser = argparse.ArgumentParser(description="hyperparameters")
    parser.add_argument('--gpu',
                        default=0,
                        type=int,
                        help='selection of gpu when you run separate trainings on single server')
    parser.add_argument('--multigpu', default=False, type=lambda x: (str(x).lower() == 'true'))
    parser.add_argument('--config_dir', type=str)
    parser.add_argument('--save_folder', type=str)
    parser.add_argument('--openset_label', type=str, help="path of openset's extra label, saved in json format")
    parser.add_argument('--openset_embedding', type=str, help="path of openset's embedding, saved in pt format")
    args = parser.parse_args()

    #set configurations
    configs = load_yaml_with_relative_ref(args.config_dir)

    print("=" * 50 + "start!!!!" + "=" * 50)

    configs = get_save_directories(configs, args.save_folder)
    # set logger
    my_logger = get_logger(
        configs["generals"]["save_folder"],
        False,
        log_level=eval("logging." + configs["generals"]["log_level"].upper()),
    )

    my_logger.logger.info("torch version is: " + str(torch.__version__))

    # set device
    device = torch.device("cuda:%d" % args.gpu if torch.cuda.is_available() else "cpu")
    torch.cuda.set_device(device)
    configs["training"]["device"] = device
    my_logger.logger.info("device: " + str(device))

    return configs, my_logger, args


def openset_eval(trainer: DASMTrainer, query: torch.Tensor):
    trainer.net.eval()
    n_test = len(trainer.test_loader)
    # score buffer
    score_buffer = dict()
    # filter type for post-processing
    filter_type = trainer.config["training"]["filter_type"] if "filter_type" in trainer.config["training"].keys(
    ) else "median"

    with torch.no_grad():
        tk = tqdm(trainer.test_loader, total=n_test, leave=False, desc="test processing")
        for batch in tk:
            wav, labels, pad_mask, idx, filename, path = batch
            wav, labels = wav.to(trainer.device), labels.to(trainer.device)
            labels_weak = (torch.sum(labels, -1) >= 1).float()
            feat = trainer.preprocess_eval(wav)
            # prediction for student
            pred = trainer.net(
                input=feat,
                pad_mask=pad_mask,
                query=query.unsqueeze(0).expand(torch.cuda.device_count(), -1, -1),
                **trainer.config[trainer.net.module.get_model_name()]["test_kwargs"],
            )
            pred = trainer.Pred(*pred)

            # =========== update psds score buffer ===========
            raw_scores, scores = batched_decode_preds(
                strong_preds=pred.strong,
                filenames=path,
                encoder=trainer.encoder,
                filter=trainer.median_fiter,
                weak_preds=None,
                need_weak_mask=False,
                filter_type=filter_type,
            )

            score_buffer.update(scores)

    # calculate psds
    test_tsv = trainer.config["dataset"]["openset_tsv"]
    test_dur = trainer.config["dataset"]["openset_dur"]
    psds_folders = trainer.config["training"]["psds_folder"]
    test_df = pd.read_csv(trainer.config["dataset"]["openset_tsv"], sep='\t')
    test_events_set = set(test_df["event_label"])
    psds, psds_single = trainer.psds(
        score_buffer,
        test_tsv,
        test_dur,
        save_dir=psds_folders,
        events_set=test_events_set,
    )

    # logging
    log_dict = OrderedDict([
        ("psds/s", psds),
    ])
    # save single psds scores
    top_10_psds = dict(sorted(psds_single.items(), key=lambda item: item[1], reverse=True)[:10])
    trainer.dict_log(top_10_psds, head="psds single class:")
    psds_single = {k: round(v, 4) for k, v in psds_single.items()}
    with open(os.path.join(trainer.config["generals"]["save_folder"], 'single_psds.json'), 'w') as f:
        json.dump(psds_single, f)

    trainer.test_log(log_dict)
    return


if __name__ == "__main__":
    configs, my_logger, args = prepare_run()

    # set network
    net = DASM(**configs["DASM"]["init_kwargs"])

    # class label dictionary
    label_dict = get_labeldict_audioset_strong(configs['dataset']['label_dict_path'])

    # set encoder (with labels for new events in the open-set dataset)
    with open(args.openset_label) as f:
        openset_label = json.load(f)
    # label_list = sorted(list(label_dict.keys())) + openset_label
    label_list = sorted(list(label_dict.keys())) + openset_label
    logging.info("<INFO> length of event label list  is {}".format(len(label_list)))
    encoder = Encoder(
        label_list,
        audio_len=configs["feature"]["audio_max_len"],
        frame_len=configs["feature"]["win_length"],
        frame_hop=configs["feature"]["hopsize"],
        net_pooling=configs["feature"]["net_subsample"],
        sr=configs["feature"]["sr"],
    )
    # set text query for the detect_any_sound
    extra_query = torch.load(args.openset_embedding)
    query = torch.cat((net.at_query, extra_query))
    logging.info("<INFO> shape of query  is {}".format(query.shape))
    # set dataloader
    devtest_df = pd.read_csv(configs["dataset"]["openset_tsv"], sep="\t")
    test_dataset = StronglyLabeledDataset(
        tsv_read=devtest_df,
        dataset_dir=configs["dataset"]["openset_folder"],
        return_name=True,
        encoder=encoder,
    )
    test_loader = DataLoader(
        test_dataset,
        batch_size=configs["training"]["batch_size_val"],
        num_workers=configs["training"]["num_workers"],
    )

    my_logger.logger.info("Total  Params: %.3f M" % (count_parameters(net, trainable_only=False) * 1e-6))
    my_logger.logger.info("Total Trainable Params: %.3f M" % (count_parameters(net) * 1e-6))

    #### move to gpus ########
    if args.multigpu:
        net = nn.DataParallel(net).to(configs["training"]["device"])
    else:
        logging.warning("Run with only single GPU!")

    trainer = DASMTrainer(
        optimizer=None,
        my_logger=my_logger,
        net=net,
        config=configs,
        encoder=encoder,
        scheduler=None,
        train_loader=None,
        val_loader=None,
        test_loader=test_loader,
        device=configs["training"]["device"],
    )
    if ("finetune_mlm" in configs['generals'].keys()) and configs['generals']["finetune_mlm"]:
        params_dict = torch.load(configs['training']["best_paths"])
        params = {
            k: v
            for k, v in params_dict.items()
            if all(keyword not in k for keyword in ["at_query", "query_projector", "at_head.", "sed_head."])
        }
        trainer.net.load_state_dict(params, strict=False)

    # load existed model
    elif configs['generals']["load_from_existed_path"]:
        trainer.net.load_state_dict(torch.load(configs['training']["best_paths"]))

    openset_eval(trainer, query)
