...
from recipes.mlm.train import train
from recipes.mlm.validation import validation
...
def main(iteration=None):
    ...
    #set encoder
    train_cfg["encoder"] = get_encoder_passt(LabelDict, configs)

    #set Dataloaders
    train_cfg = get_datasets_mlm(configs, train_cfg)

    #set network
    train_cfg["net"] = get_models_mlm(configs)
    ...
    patch_transformer_params = list(map(id, train_cfg["net"].patch_transformer.parameters()))
    ...
    if not train_cfg["test_only"]:
        ...
        for train_cfg["epoch"] in range(train_cfg["n_epochs"]):
            ...
            if val_metrics < best_val_metrics:
                ...
                torch.save(stud_best_state_dict, train_cfg["best_paths"][0])
                best_val_metrics = val_metrics
            # save_list = [10, 15]
            # if train_cfg["epoch"] in save_list:
            #     os.rename(train_cfg["best_paths"][0], train_cfg["best_paths"][0][:-3] + str(train_cfg["epoch"]) + ".pt")
